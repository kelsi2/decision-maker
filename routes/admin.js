...
module.exports = function (database) {
  ...
  router.get("/:pollId", (req, res) => {
    ...
    database.getUsersThatVotedFromId(pollId)
    .then((users) => {
      //console.log(users);
      return Promise.all( users.map(name => {
        return database.getRankFromUser(name.user_id, pollId)
        .then((ranks) => {
    ...
    .then(ranksArray => {
      ...
      return database.getPoll(pollId)
      .then((result) => {
        ...
        .then((object) => {
          ...
          console.log(util.inspect(templateVars, {showHidden: false, depth: 4}))
